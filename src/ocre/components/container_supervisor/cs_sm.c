...
#if OCRE_CS_DEBUG_ON
			LOG_INF("Transitioning from state STATE_RUNTIME_UNINITIALIZED_RUN to state "
...
static enum smf_state_result runtime_running_run(void *o)
{
	...
	switch (msg->event) {
		...
		case EVENT_STOP_CONTAINER: {
			...
			if (CS_stop_container(&ctx->containers[msg->containerId], callback) ==
		...
		case EVENT_DESTROY_CONTAINER: {
			...
			if (CS_destroy_container(&ctx->containers[msg->containerId], callback) ==
		...
		case EVENT_RESTART_CONTAINER: {
			...
			if (CS_restart_container(&ctx->containers[msg->containerId], callback) ==
