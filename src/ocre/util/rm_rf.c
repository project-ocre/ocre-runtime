...
int rm_rf_verbose(const char *path, int (*error_callback)(const char *path, int error_code)) {
    ...
    while (stack) {
        ...
        if (current->is_removal_phase) {
            ...
            if (rmdir(current->path) == -1) {
                if (error_callback) {
                    error_callback(current->path, errno);
                }
                result = -1;
            }
            stack = pop_dir(stack);
        ...
        if (!entry_path) {
            if (error_callback) {
                error_callback("memory allocation", ENOMEM);
            }
            result = -1;
            errno = ENOMEM;
            break;
        ...
        if (stat(entry_path, &st) == -1) {
            if (error_callback) {
                error_callback(entry_path, errno);
            }
            result = -1;
            free(entry_path);
            continue;
        ...
        if (S_ISDIR(st.st_mode)) {
            ...
            if (!new_node) {
                if (error_callback) {
                    error_callback(entry_path, ENOMEM);
                }
                result = -1;
                errno = ENOMEM;
                free(entry_path);
