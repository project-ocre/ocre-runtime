...
int main(int argc, char *argv[])
{
	...
	if (ret == RUNTIME_STATUS_INITIALIZED) {
		...
		do {
			...
			snprintf(ocre_container_data[i].sha256, sizeof(ocre_container_data[i].sha256), "%s",
				 container_filename);
			ocre_container_data[i].timers = 0;
			ocre_container_data[i].watchdog_interval = 0;
			ocre_container_runtime_create_container(&ctx, &ocre_container_data[i], &container_ID[i],
								callback[i]);
