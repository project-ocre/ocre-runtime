 ...
 *
 * These functions are guaranteed to be operated according to the Ocre Container lifecycle
 * specification.
 *
...
struct ocre_runtime_vtable {
	 ...
	 *
	 * It should be unique among all registered runtime engines. Please refer to the full
	 * documentation for the list of assigned runtime engine names.
	 */
	 ...
	 *
	 * This function is called when the runtime engine is initialized. It is called only once on
	 * start and before any other functions are called.
	 *
	 ...
	 *
	 * This function will return an opaque pointer to the context of the runtime instance. It
	 * can be anything managed internally by the runtime. It cannot be NULL, as it will indicate
	 * error. This value is passed as a parameter to the other functions in this interface.
	 *
	 ...
	 * @param argv A NULL-terminated array of command line arguments to be passed to the
	 * container
	 ...
	 *
	 * This function is called when a runtime instance is destroyed. This is guaranteed to be
	 * called only when the container should not be running or paused.
	 *
	 ...
	 *
	 * This function is called inside a thread when the container is started. It should execute
	 ...
	 *
	 * This function is called when a runtime instance is requested to stop. This is guaranteed
	 * to be called only when the container is running or paused.
	 *
	 * @param runtime_context Pointer to the runtime context
	 * @return 0 on success, non-zero on failure
	 */
	 ...
	 *
	 * This function is called when a runtime instance is requested to be killed. This is
	 * guaranteed to be called only when the container is running. This should terminate the
	 ...
	 *
	 * This function is called when a runtime instance is requested to be paused. This is
	 * guaranteed to be called only when the container is running.
	 *
	 * @param runtime_context Pointer to the runtime context
	 * @return 0 on success, non-zero on failure
	 */
	 ...
	 *
	 * This function is called when a runtime instance is requested to be unpaused. This is
	 * guaranteed to be called only when the container is paused.
	 *
	 * @param runtime_context Pointer to the runtime context
	 * @return 0 on success, non-zero on failure
	 */
